...
import { restoreErrorFromString } from "../helpers";
  ...
  private _installGlobalOnErrorHandler(): void {
    ...
    if (!!sdk.onError) {
      ...
      sdk.onError((err: string | object) => {
        ...
        const error = typeof err === 'string' ? restoreErrorFromString(err) : err
    ...
    if (!!sdk.onUnhandledRejection) {
      ...
      sdk.onUnhandledRejection(
        ({ reason, promise }: OnUnhandledRejectionRes) => {
          ...
          const error = typeof reason === 'string' ? restoreErrorFromString(reason) : reason
          currentHub.captureException(error, {
            data: promise,
          });
        }
